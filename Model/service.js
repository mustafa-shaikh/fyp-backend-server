...
async function _publishSymptoms() {
  let options = {
    method: "getinfo",
  };
  let response = await fetch(chainUri, {
...
async function _publishPrescriptions() {
  let options = {
    method: "publish",
    id: 1,
    params: ["symptoms", "123", symptoms],
    chain_name: "chain1",
  };
  let response = await fetch(chainUri, {
    method: "POST",
...
// chain1 publish "symptoms" $count '{"json":'$i'}'

async function _getPred(data) {
  let response = await fetch(modelUri, {
    method: "POST",
